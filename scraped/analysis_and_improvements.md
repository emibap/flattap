...
1. **Convert disc-units to km for display** — on the azimuthal equidistant projection each SVG unit = `(Math.PI * 6371) / DISC_RADIUS` km. Show "Off by ~1,450 km" instead of "Off by 0.38 disc-units".
2. **Add round multipliers** (rounds 3×2, 4-5×3) to raise max to 1000 and create a satisfying score escalation — the visual colour change per round makes this feel rewarding rather than punishing.
3. **Consider haversine** — the flat-earth theme actually makes Euclidean distance thematically appropriate, but labelling it in km is still useful. Keep Euclidean internally, just convert the displayed readout.
...
1. **Expand to a themed 30–45 emoji gradient** — keep the flat-earth comedy but give more variety. Example gradient from bad to perfect:  
   `🌍🌍😵🤢😭🤬🥺😒😥😪🥶😶😕😞😨😟😔🤫😑😐🙃🙂😁😂🌞🗺️🧭🎯🏅🏆`
2. **Use `floatScore` not integer score** as the index — scores of 89 and 91 currently get very different emojis but a continuous system makes the gradient feel fair.
...
### Suggested improvements
1. **Add game number** once daily mode exists: `"Flattap #12 · April 12"`
2. **Show city names** (or short names) in the final round breakdown so the viewer can see what was guessed — Maptap doesn't do this but it's more fun
3. **Remove `/ 500`** from share text — it gives away the scale and makes low scores look worse; Maptap just shows the raw number
...
2. **Add a `finalGradeFlavor(score)` equivalent** for the end-game total — Maptap shows "Geographic Master!" etc. for the final screen.
...
1. **Daily mode with deterministic seeding** — derive a seed from the date and use it to pick cities:
   ...
   ```
2. **Persist game state** in localStorage with a YYYY-MM-DD key — mid-game refresh resumes where the player left off.
3. **One-play-per-day** — detect `round >= 6` in localStorage and jump to the final screen with a "Come back tomorrow" message and a `timeToMidnight()` countdown (Maptap has this function in jjbasics.js verbatim).
4. **Game number in header** — `"Flattap #N · April 12"` where N = days since launch.
5. **Keep a "Practice / Random" mode** alongside daily — a simple second button below the board.
...
1. **localStorage `flattap_stats`** — same pattern as Maptap, no accounts needed:
   ...
   ```
2. **Show streak in header** — "🔥 5" next to the round counter. Reset if player misses a day.
3. **Personal best banner** — on the final overlay, show "🏆 New personal best!" if `finalScore > gameHistory.highScore`.
4. **Score distribution buckets** — track `over900`, `over800` etc. to show on a simple "Your stats" modal later.
...
### Suggested improvements
1. **Round progress dots** — add 5 emoji/SVG dots to Flattap's stats bar. The current "Round X / 5" text works but dots are more glanceable on mobile.
2. **Colour the prompt bar** by round — a subtle background tint shift as difficulty escalates is great UX signalling.
...
### Suggested improvements
1. **Animated result line** — use SVG `stroke-dashoffset` animation to draw the dashed line from guess to target rather than having it pop in instantly.
2. **Score number bounce** — add a CSS `@keyframes` scale/bounce on the `+score` number when the toast appears.
3. **"Drip stars" on high scores** — when a round scores ≥ 85, drop 3–5 ⭐ characters from the score counter using the same CSS animation pattern Maptap uses.
4. **Shooting comet on first play or return** — Maptap's CometEffect is self-contained and fully portable (~300 lines). It's a delightful surprise that costs no ongoing CPU.
5. **Fireworks on final score ≥ 800** — the canvas-based fireworks in `effects.js` are also self-contained and paused when tab is hidden.
...
### Suggested improvements
1. **Confirm tap mode** — show a draggable pin after tap, with a "Confirm" and "Clear" button. On mobile, fat fingers regularly misfire on a small map. This is the single highest-impact mobile UX improvement possible.
2. Implement as: on click, show a pin SVG circle at click coords, enable the button pair, second click on Confirm calls `resolveRound`.
...
### Suggested improvements for Flattap
...
This is a **golden feature** for Flattap. The flat-earth equivalent could be: if your total score is below some threshold, trigger a "**You found the Ice Wall!**" mode — check how close your guesses were to the outer rim of the disc and score that instead. Award a special "🧊 Ice Wall Hunter" badge.
...
### Suggested improvements
1. **Adopt the Maptap schema** exactly — `id, name, lat, lng, continent, type, difficulty`. The full `master_locations.js` is already in `maptap-inspiration/`.
2. **Expand to 300–500 locations** — include 30+ each of islands, monuments, natural wonders, national parks. These are visually distinctive on the flat-earth disc and create memorable moments.
3. **Names with country context** — "Machu Picchu, Peru" is more interesting and teaches geography.
4. **Use `difficulty` for daily seeding** — ensure each daily game has a mix of difficulties rather than 5 random picks.
5. **Use `continent` for balance** — try to spread the 5 daily locations across different continents.
...
### Suggested improvements
1. **Add OG + Twitter meta tags** — 30 minutes of work, massive impact on share virality.
2. **Fix the title typo** — "Flatap" vs "Flattap" in the `<title>` tag.
3. **Add a banner image** — a flat-earth disc with pins on it makes for a great social share preview.
